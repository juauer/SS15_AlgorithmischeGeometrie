...
Die parallele Gerade $h$ durch den Punkt $p$ ist gegeben als $h:= \overrightarrow{n}(\overrightarrow{x}-\overrightarrow{p})=0$ (Normalform). Die zu $g$ orthogonale Gerade $i$, welche durhc den Koordinatenursprung verläuft ist gegeben als $i:=\overrightarrow{x}=s\overrightarrow{n}$ (Parameterform).
